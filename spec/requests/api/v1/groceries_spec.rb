...
RSpec.describe "Api::V1::Groceries", type: :request do
  ...
  describe "POST /create" do
    it "admin succes to post grocery" do
      ...
      post api_v1_groceries_path, params: params, headers: auth_tokens
  ...
  describe "PATCH/PUT /update" do
    it "admin succes to update 1 grocery" do
      ...
      put api_v1_grocery_path(grocery.id), params: params, headers: auth_tokens
      expect(response).to have_http_status :ok
    end
    ...
    it "user succes to update 2 grocery" do
      ...
      put api_v1_grocery_path(grocery2.id), params: params, headers: auth_tokens
      expect(response).to have_http_status :ok
    end
  end
  ...
  describe "DELETE /destroy" do
    it "admin succes to delete 1 grocery" do
      auth_tokens = sign_in(admin)
      delete api_v1_grocery_path(grocery.id), headers: auth_tokens
      expect(response).to have_http_status :ok
    end

    it "user succes to delete 2 grocery" do
      auth_tokens = sign_in(user2)
      delete api_v1_grocery_path(grocery2.id), headers: auth_tokens
      expect(response).to have_http_status :ok
    end
  end
end
