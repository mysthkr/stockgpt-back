...
RSpec.describe "Api::V1::Makers", type: :request do
  ...
  describe "POST /create" do
    it "admin succes to post maker" do
      ...
      post api_v1_makers_path, params: params, headers: auth_tokens
  ...
  describe "PATCH/PUT /update" do
    it "admin succes to update 1 maker" do
      ...
      put api_v1_maker_path(maker.id), params: params, headers: auth_tokens
      expect(response).to have_http_status :ok
    end
    ...
    it "user succes to update 2 maker" do
      ...
      put api_v1_maker_path(maker2.id), params: params, headers: auth_tokens
      expect(response).to have_http_status :ok
    end
  end
  ...
  describe "DELETE /destroy" do
    it "admin succes to delete 1 maker" do
      auth_tokens = sign_in(admin)
      delete api_v1_maker_path(maker.id), headers: auth_tokens
      expect(response).to have_http_status :ok
    end

    it "user succes to delete 2 maker" do
      auth_tokens = sign_in(user2)
      delete api_v1_maker_path(maker2.id), headers: auth_tokens
      expect(response).to have_http_status :ok
    end
  end
end
