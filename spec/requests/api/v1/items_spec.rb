...
RSpec.describe "Api::V1::Items", type: :request do
  ...
  describe "GET /show" do
    it "admin succes to get 1 item" do
      auth_tokens = sign_in(admin)
      get api_v1_items_path(item.id), headers: auth_tokens
      expect(response).to have_http_status :ok
    end

    it "user succes to get 2 item" do
      auth_tokens = sign_in(user2)
      get api_v1_items_path(item2.id), headers: auth_tokens
      expect(response).to have_http_status :ok
    end
  end
  ...
  describe "POST /create" do
    it "admin succes to post item" do
      ...
      post api_v1_items_path, params: params, headers: auth_tokens
  ...
  describe "PATCH/PUT /update" do
    it "admin succes to update 1 item" do
      ...
      put api_v1_item_path(item.id), params: params, headers: auth_tokens
      expect(response).to have_http_status :ok
    end
    ...
    it "user succes to update 2 item" do
      ...
      put api_v1_item_path(item2.id), params: params, headers: auth_tokens
      expect(response).to have_http_status :ok
    end
  end
  ...
  describe "DELETE /destroy" do
    it "admin succes to delete 1 item" do
      auth_tokens = sign_in(admin)
      delete api_v1_item_path(item.id), headers: auth_tokens
      expect(response).to have_http_status :ok
    end

    it "user succes to delete 2 item" do
      auth_tokens = sign_in(user2)
      delete api_v1_item_path(item2.id), headers: auth_tokens
      expect(response).to have_http_status :ok
    end
  end
end
