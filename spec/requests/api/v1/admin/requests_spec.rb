...
RSpec.describe "Api::V1::Admin::Requests", type: :request do
  ...
  describe "GET /show" do
    it "admin succes to get 1 request" do
      auth_tokens = sign_in(admin)
      get api_v1_admin_request_path(request.id), headers: auth_tokens
      expect(response).to have_http_status :ok
    end

    it "user fail to get 2 request" do
      auth_tokens = sign_in(user2)
      get api_v1_admin_request_path(request2.id), headers: auth_tokens
      expect(response).to have_http_status :unauthorized
    end
  end
  ...
  describe "PATCH/PUT /update" do
    it "admin succes to update 1 request" do
      ...
      put api_v1_admin_request_path(request.id), params: params, headers: auth_tokens
      expect(response).to have_http_status :ok
    end
  ...
  describe "DELETE /destroy" do
    it "admin succes to delete 1 request" do
      auth_tokens = sign_in(admin)
      delete api_v1_admin_request_path(request.id), headers: auth_tokens
      expect(response).to have_http_status :ok
    end
