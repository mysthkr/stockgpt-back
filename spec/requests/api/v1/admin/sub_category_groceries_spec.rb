...
RSpec.describe "Api::V1::Admin::SubCategoryGroceries", type: :request do
  ...
  describe "GET /show" do
    it "admin succes to get 1 sub_category_grocery" do
      auth_tokens = sign_in(admin)
      get api_v1_admin_sub_category_grocery_path(sub_category_grocery.id), headers: auth_tokens
      expect(response).to have_http_status :ok
    end

    it "user fail to get 2 sub_category_grocery" do
      auth_tokens = sign_in(user2)
      get api_v1_admin_sub_category_grocery_path(sub_category_grocery2.id), headers: auth_tokens
      expect(response).to have_http_status :unauthorized
    end
  end
  ...
  describe "PATCH/PUT /update" do
    it "admin succes to update 1 sub_category_grocery" do
      ...
      put api_v1_admin_sub_category_grocery_path(sub_category_grocery.id), params: params, headers: auth_tokens
      expect(response).to have_http_status :ok
    end
  ...
  describe "DELETE /destroy" do
    it "admin succes to delete 1 sub_category_grocery" do
      auth_tokens = sign_in(admin)
      delete api_v1_admin_sub_category_grocery_path(sub_category_grocery.id), headers: auth_tokens
      expect(response).to have_http_status :ok
    end
