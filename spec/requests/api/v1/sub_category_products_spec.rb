...
RSpec.describe "Api::V1::SubCategoryProducts", type: :request do
  ...
  describe "GET /show" do
    it "admin succes to get 1 sub_category_product" do
      auth_tokens = sign_in(admin)
      get api_v1_sub_category_products_path(sub_category_product.id), headers: auth_tokens
      expect(response).to have_http_status :ok
    end

    it "user succes to get 2 sub_category_product" do
      auth_tokens = sign_in(user2)
      get api_v1_sub_category_products_path(sub_category_product2.id), headers: auth_tokens
      expect(response).to have_http_status :ok
    end
  end
  ...
  describe "POST /create" do
    it "admin succes to post sub_category_product" do
      ...
      post api_v1_sub_category_products_path, params: params, headers: auth_tokens
  ...
  describe "PATCH/PUT /update" do
    it "admin succes to update 1 sub_category_product" do
      ...
      put api_v1_sub_category_product_path(sub_category_product.id), params: params, headers: auth_tokens
      expect(response).to have_http_status :ok
    end
    ...
    it "user succes to update 2 sub_category_product" do
      ...
      put api_v1_sub_category_product_path(sub_category_product2.id), params: params, headers: auth_tokens
      expect(response).to have_http_status :ok
    end
  end
  ...
  describe "DELETE /destroy" do
    it "admin succes to delete 1 sub_category_product" do
      auth_tokens = sign_in(admin)
      delete api_v1_sub_category_product_path(sub_category_product.id), headers: auth_tokens
      expect(response).to have_http_status :ok
    end

    it "user succes to delete 2 sub_category_product" do
      auth_tokens = sign_in(user2)
      delete api_v1_sub_category_product_path(sub_category_product2.id), headers: auth_tokens
      expect(response).to have_http_status :ok
    end
  end
end
